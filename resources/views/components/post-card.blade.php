<div class="card @if(!request()->routeIs('post.public.view')) h-full @endif dark:bg-base-200 bg-@isset($type){{str_replace('500', '100',$post->type->color)}}@else{{str_replace('500', '100',$post->course->color)}}@endif">
  <figure> 
    ...
    @elseif(true)
    <div class="relative inline-block border-b-4 border-@isset($type){{$post->type->color}}@else{{$post->course->color}}@endif">
    <svg
   class='dark:text-base-100 text-@isset($type){{$post->type->color}}@else{{$post->course->color}}@endif  object-cover w-full h-48'
...
<span class="absolute inset-0 flex items-center justify-center font-bold text-xl pointer-events-none">
  ...
  <div class="card-body">
    <h2 class="card-title">{{$post->title}}</h2> @if($description === true) <p>{{$post->description}}</p> @endif <div class="card-actions items-center justify-center flex">
      <div class="items-center justify-center flex min-h-24 min-w-[18rem] max-w-4xl flex-wrap gap-2">
        <ul class="menu menu-horizontal bg-base-100 rounded-box">
          ...
          </li> @foreach($post->files as $file) @if ($file->type == 'primary light') <li>
          ...
          </li> @elseif($file->type == 'primary dark') <li>
            <a href="{{url('storage/'.$file->file_path.'')}}">
              <svg fill="currentColor" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
                <path d="M480-120q-150 0-255-105T120-480q0-150 105-255t255-105q14 0 27.5 1t26.5 3q-41 29-65.5 75.5T444-660q0 90 63 153t153 63q55 0 101-24.5t75-65.5q2 13 3 26.5t1 27.5q0 150-105 255T480-120Zm0-80q88 0 158-48.5T740-375q-20 5-40 8t-40 3q-123 0-209.5-86.5T364-660q0-20 3-40t8-40q-78 32-126.5 102T200-480q0 116 82 198t198 82Zm-10-270Z" />
              </svg>
            </a>
          </li> @endif @endforeach @if($post->decks->count() > 0) <li>
          ...
          </li> @endif @if($post->quizlet_url !== NULL) <li>
    ...
    <div class="flex flex-items"> @isset($type) <a class="badge badge-md bg-{{$post->type->color}} text-white">{{$post->type->name}}</a> @else <a wire:navigate href="{{route('post.public.courseView', ['curriculum_chosen' => $post->level->curriculum->slug, 'level_chosen' => $post->level->slug, 'course_chosen' => $post->course->slug])}}" class="badge badge-md bg-{{$post->course->color}} text-white">{{$post->course->name}}</a> @endisset @if($post->pinned) <span class="ml-1 badge badge-md badge-neutral">{{__('Pinned')}}</span> @endif @if($showGroup === true) @if($post->group_id == 2) <span class="ml-1 badge badge-md badge-neutral">
        <svg fill="currentColor" xmlns="http://www.w3.org/2000/svg" height="17" viewBox="0 -960 960 960" width="24">
          <path d="M480-80q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm-40-82v-78q-33 0-56.5-23.5T360-320v-40L168-552q-3 18-5.5 36t-2.5 36q0 121 79.5 212T440-162Zm276-102q20-22 36-47.5t26.5-53q10.5-27.5 16-56.5t5.5-59q0-98-54.5-179T600-776v16q0 33-23.5 56.5T520-680h-80v80q0 17-11.5 28.5T400-560h-80v80h240q17 0 28.5 11.5T600-440v120h40q26 0 47 15.5t29 40.5Z" />
        </svg> {{__('Public')}}
      </span> @elseif($post->group_id == 1) <span class="ml-1 badge badge-md badge-neutral">
        <svg fill="currentColor" xmlns="http://www.w3.org/2000/svg" height="17" viewBox="0 -960 960 960" width="24">
          <path d="M240-80q-33 0-56.5-23.5T160-160v-400q0-33 23.5-56.5T240-640h40v-80q0-83 58.5-141.5T480-920q83 0 141.5 58.5T680-720v80h40q33 0 56.5 23.5T800-560v400q0 33-23.5 56.5T720-80H240Zm0-80h480v-400H240v400Zm240-120q33 0 56.5-23.5T560-360q0-33-23.5-56.5T480-440q-33 0-56.5 23.5T400-360q0 33 23.5 56.5T480-280ZM360-640h240v-80q0-50-35-85t-85-35q-50 0-85 35t-35 85v80ZM240-160v-400 400Z" />
        </svg>{{__('Private')}}
      </span> @else <span class="ml-1 badge badge-md badge-neutral">
        <svg fill="currentColor" xmlns="http://www.w3.org/2000/svg" height="17" viewBox="0 -960 960 960" width="24">
        ...
        </svg> {{$post->group->name}} @endif @endif </div>
  </div>
</div>