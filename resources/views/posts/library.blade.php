...
<p>{{__('Taught at')}} @foreach($curriculum->schools as $school) {{$school->name}} @endforeach</p>
  ...
  @foreach($level->courses as $course)
            <div wire:navigate href="{{route('post.public.courseView', ['curriculum_chosen' => $curriculum->slug, 'level_chosen' => $level->slug, 'course_chosen' => $course->slug])}}" class="cursor-pointer col-span-1 flex rounded-md shadow-sm border-{{$course->color}} border-2">
            <div class="flex w-16 flex-shrink-0 items-center justify-center bg-{{str_replace('500', '500',$course->color)}} rounded-l-smd text-sm font-medium text-white">{{strtoupper(mb_substr($course->name, 0, 1))}}</div>
