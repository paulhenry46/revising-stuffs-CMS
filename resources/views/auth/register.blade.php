...
<div class='inline-block text-4xl mt-10 ml-10 text-gray-100 font-bold bg-gray-900/50 p-5 rounded-md'>
    {{ __('Ready to dive in?') }}</br>
    {{ __('Start revising now !') }}
</div>
<div class="absolute bottom-10 mb-10 mr-10 right-0 text-4xl mt-10 ml-10 text-gray-100 font-bold bg-gray-900/50 p-5 rounded-md">
    {{ env('APP_NAME') }}
  </div>
    </div>
    <div>
        ...
        @if(isset($contributorDomainRules) && $contributorDomainRules->isNotEmpty())
        <div role="alert" class="alert alert-info mb-4">
            ...
            <div>
                <p class="font-semibold">{{ __('Automatic access') }}</p>
                <p class="text-sm">{{ __('Users with one of the following email domains will automatically be granted contributor access (no prior moderation for posts and comments) upon verifying their address:') }}</p>
                <ul class="mt-1 list-disc list-inside text-sm">
                    @foreach($contributorDomainRules as $rule)
                    <li class="font-mono">{{ $rule->domain }}</li>
                    @endforeach
                </ul>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="flex items-center justify-end mt-4">
                <a wire:navigate class="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" href="{{ route('login') }}">
        ...
        @if(env('GOOGLE_AUTH'))
        <div class='divider divider-neutral'>{{ __('OR') }}</div>
        <a href='{{ route('login.google') }}' class=' btn btn-outline w-full'> {{__('Login with Google')}}<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 48 48">
