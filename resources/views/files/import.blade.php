...
<div class="border-b dark:border-white/10 border-gray-900/10 pb-12">
  ...
  <input disabled name="name" id="name" disabled type="text" placeholder="Title will be the name of the file" class="input input-bordered w-full " />
</label>
    </div>
  </div>
</div>
<div class="border-b dark:border-white/10 border-gray-900/10 pb-12">
  <h2 class="text-base font-semibold leading-7 text-gray-900 dark:text-white">{{__('Upload your files')}}</h2>
  <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
  <div class="col-span-full">
      <label for="file" class="block text-sm font-medium leading-6 dark:text-white text-gray-900">{{__('Your files')}}</label>
      <div class="mt-2">
        <input type="file" name="files[]" multiple class="file-input file-input-sm file-input-bordered w-full max-w-xs" />
      </div>
    </div>
  </div>
</div>
